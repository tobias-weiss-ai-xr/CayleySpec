...
The project formalizes three interconnected domains: (1) vertex-transitivity of Cayley graphs, (2) the spectral decomposition of Cayley adjacency operators via group characters and the Peter--Weyl theorem for finite groups, and (3) the definition and SL(2,\mathbb{Z})-invariance of Hecke operators acting on modular forms.

The formalization consists of approximately 1,000 lines of Lean code across five modules and builds successfully against mathlib4 v4.31.0 (3,265 jobs, 0 errors).
All core theorems are fully proved except for the boundedness-at-cusps condition of the Hecke operator, which requires q-expansion machinery not yet present in mathlib.

The project serves dual purposes: as a self-contained case study in formalizing number-theoretic constructions in Lean, and as the formal-methods foundation for a broader research program connecting graph neural network expressivity to number-theoretic complexity via SL(2,\mathbb{F}_p) Cayley graphs.
\end{abstract}
...
More concretely, the eigenvalues of Hecke operators---linear operators acting on spaces of modular forms---appear as the spectra of certain Cayley graphs of GL(2,\mathbb{Z}/p\mathbb{Z})~\cite{davidoff2003elementary}.
...
Section~\ref{sec:evaluation} evaluates the project: build statistics, what is proved versus admitted, and lessons learned.
...
This is captured by the type \texttt{reps (n : ℤ)} with a \texttt{Fintype} instance from mathlib.

\subsubsection{Slash-Invariance Under Reduction}

The key lemma states that SL(2,\mathbb{Z})-invariant functions are unaffected by the reduction algorithm on fixed-determinant matrices:
...
The proof uses \texttt{exists\_smul\_reduce}: every fixed-determinant matrix is in the SL(2,\mathbb{Z})-orbit of its reduction.
...
\begin{enumerate}
  ...
  \item \texttt{reps\_inj}: if $A,B\in\mathrm{reps}(m)$ are in the same SL(2,\mathbb{Z})-coset, then $A=B$ (via entrywise comparison using bounds)
\end{enumerate}

\subsubsection{SL(2,\mathbb{Z})-Invariance of the Hecke Operator}
...
\subsubsection{Admitted Condition}

The boundedness-at-cusps condition is permanently admitted.
This condition requires the theory of $q$-expansions---expressing modular forms as Fourier series $f(\tau)=\sum a_n q^n$ with $q=e^{2\pi i\tau}$---which would allow checking boundedness at the cusp $\infty$ by examining the coefficients $a_n$.
While mathlib has Fourier theory for finite groups, the $q$-expansion machinery for modular forms is not yet present and constitutes a substantial formalization project in its own right.
...
\midrule
Total build jobs & 3,265 \\
Errors & 0 \\
Admitted theorems & 1 (boundedness at cusps) \\
...
\subsection{What is Proved, What is Admitted}

All core theorems are fully proved:
...
\begin{itemize}
  ...
  \item \texttt{slash\_invariant\_under\_reduce}: SL(2,\mathbb{Z})-invariant functions are invariant under the reduction of fixed-determinant matrices.
  \item \texttt{heckeOperator\_slash}: the Hecke operator $T_n$ preserves SL(2,\mathbb{Z})-invariance.
  \item \texttt{heckeOperator\_modularForm}: $T_n$ maps modular forms to modular forms (holomorphy proved, boundedness admitted).
\end{itemize}

The only admitted theorem is the boundedness-at-cusps condition for the Hecke operator, which requires $q$-expansion theory not yet available in mathlib.
...
\textbf{Congruence and sums.} Earlier versions of the \texttt{heckeOperator\_slash} proof attempted to use \texttt{congr} or \texttt{w} to rewrite inside a sum binder. The working approach was to apply \texttt{congr 1; funext M; exact \dots}, which works because after \texttt{congr 1}, the goal is an equality of functions at each $M$.

\textbf{Matrix algebra.} The $\varphi\_inj$ proof required careful handling of matrix equations with SL(2,\mathbb{Z}) elements. The key trick was to use \texttt{ext : 1} to descend to the matrix level, then use \texttt{Matrix.mul\_assoc} and right-cancellation via $\gamma^{-1}$.

\textbf{Induction on the reduction algorithm.} The \texttt{exists\_smul\_reduce} lemma required induction on \texttt{reduce\_rec} with three cases. Each case required explicit construction of the SL(2,\mathbb{Z}) element and a calc chain verifying the equality. The modular group generators $T$ (translation) and $S$ (inversion) were essential.
...
\begin{itemize}
  ...
  \item SL(2,\mathbb{Z})-invariance and holomorphy of Hecke operators (513 lines).
\end{itemize}

The build is clean (0 errors, 1 permanently admitted theorem) and the code serves as both a self-contained formalization and a foundation for future work.

\paragraph{Future work.}
Several directions are planned:

\begin{enumerate}
  \item \textbf{$q$-expansion theory}: The most immediate technical gap is the $q$-expansion machinery for modular forms in mathlib. This would close the boundedness-at-cusps condition and complete the full proof of the modular form property for Hecke operators.
  \item \textbf{Spectra of SL(2,$\mathbb{F}_p$) Cayley graphs}: The formalization of the Peter--Weyl theorem for finite groups can be extended to compute the spectrum of $\mathrm{SL}(2,\mathbb{F}_p)$ Cayley graphs via the character table of $\mathrm{SL}(2,\mathbb{F}_p)$.
